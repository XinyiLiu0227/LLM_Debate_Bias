...
if __name__ == "__main__":
    ...
    if os.path.exists(f'experiments/{file_prefix}_{model_name}_{args.seed}_{args.debate_num}_remove_forfeit_change_names_AB_lose.json'):
        with open(f'experiments/{file_prefix}_{model_name}_{args.seed}_{args.debate_num}_remove_forfeit_change_names_AB_lose.json', 'r') as f:
            for line in f:
    ...
    dir_name = f'{file_prefix}_{model_name}_remove_forfeit_change_names_AB_lose_log'
    ...
    for debate_name in selected_debate_names:
        ...
        if prob > 0.5:
            ...
            instruction =   f"Assume you are a debate evaluator, there are two participants in this debate. Given the above context of the whole debate. \
                    please give the decision on which participant is the loser, \
                    you only need to give the character of either {masked_name[pro_index]}, or {masked_name[con_index]}. {masked_name[pro_index]} means user {masked_name[pro_index]} loses. {masked_name[con_index]} means user {masked_name[con_index]} loses. \
                    Please only give the result without any other words."
            new_instruction =  f'Assume you are the debate evaluator, there are two participants in this debate, \
                    after given the context of the debate, please give the decision on which participant is the loser in terms of making more convincing arguments, \
                    without considering the grammar and spelling, the reliability of the source and the debater conduct. \
                    You only need to give the character of either {masked_name[pro_index]} or {masked_name[con_index]}, {masked_name[pro_index]} means the user {masked_name[pro_index]} loses, {masked_name[con_index]} means user {masked_name[con_index]} loses. \
                    Please only give the result without any other words.'
        else:
            instruction =   f"Assume you are a debate evaluator, there are two participants in this debate. Given the above context of the whole debate. \
                    please give the decision on which participant is the loser, \
                    you only need to give the character of either {masked_name[con_index]}, or {masked_name[pro_index]}. {masked_name[con_index]} means user {masked_name[con_index]} loses. {masked_name[pro_index]} means user {masked_name[pro_index]} loses. \
                    Please only give the result without any other words"
            new_instruction =  f'Assume you are the debate evaluator, there are two participants in this debate, \
                    after given the context of the debate, please give the decision on which participant is the loser in terms of making more convincing arguments, \
                    without considering the grammar and spelling, the reliability of the source and the debater conduct. \
                    You only need to give the character of either {masked_name[con_index]} or {masked_name[pro_index]}, {masked_name[con_index]} means the user {masked_name[con_index]} wins, {masked_name[pro_index]} means user {masked_name[pro_index]} loses. \
                    Please only give the result without any other words.'   
        
        for i, round in enumerate(current_debate['rounds']):
            for side in round:
                cur_side = side['side']
                if cur_side == 'Cry Me a River,':
                    cur_side = 'Con'
                if i == 0:
                    try:
        ...
        with open(f'experiments/{file_prefix}_{model_name}_{args.seed}_{args.debate_num}_remove_forfeit_change_names_AB_lose.json', 'a') as f:
