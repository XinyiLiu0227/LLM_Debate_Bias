...
if __name__ == "__main__":
    ...
    # if proviede a file, then use the debates from the file
    if args.file:
    ...
    if os.path.exists(f'experiments/{file_prefix}_{model_name}_{args.seed}_{args.debate_num}_remove_forfeit_change_names_1-1_reversed.json'):
        with open(f'experiments/{file_prefix}_{model_name}_{args.seed}_{args.debate_num}_remove_forfeit_change_names_1-1_reversed.json', 'r') as f:
            for line in f:
    ...
    dir_name = f'{file_prefix}_{model_name}_remove_forfeit_change_names_1-1_reversed_log'
    ...
    for debate_name in selected_debate_names:
        ...
        # prob = random.uniform(0, 1) # shuffle the position of Pro and Con
        ...
        if prob > 0.5:
            ...
            #         you only need to give the number of either 1, or -1. 1 means user {masked_name[pro_index]} wins. -1 means user {masked_name[con_index]} wins. \
            #         Please only give the result without any other words"
            instruction =   f"Assume you are a debate evaluator, there are two participants in this debate. Given the above context of the whole debate. \
                    please give the decision on which participant is the winner, \
                    you only need to give the number of either {masked_name[pro_index]}, or {masked_name[con_index]}. {masked_name[pro_index]} means user {masked_name[pro_index]} wins. {masked_name[con_index]} means user {masked_name[con_index]} wins. \
                    Please only give the result without any other words."
            new_instruction =  f'Assume you are the debate evaluator, there are two participants in this debate, \
                    ...
                    You only need to give the number of either {masked_name[pro_index]} or {masked_name[con_index]}, {masked_name[pro_index]} means the user {masked_name[pro_index]} wins, {masked_name[con_index]} means user {masked_name[con_index]} wins. \
                    Please only give the result without any other words.'
        else:
            instruction =   f"Assume you are a debate evaluator, there are two participants in this debate. Given the above context of the whole debate. \
                    please give the decision on which participant is the winner, \
                    you only need to give the number of either {masked_name[con_index]}, or {masked_name[pro_index]}. {masked_name[con_index]} means user {masked_name[con_index]} wins. {masked_name[pro_index]} means user {masked_name[pro_index]} wins. \
                    Please only give the result without any other words"
            new_instruction =  f'Assume you are the debate evaluator, there are two participants in this debate, \
                    after given the context of the debate, please give the decision on which participant is the winner in terms of making more convincing arguments, \
                    without considering the grammar and spelling, the reliability of the source and the debater conduct. \
                    You only need to give the number of either {masked_name[con_index]} or {masked_name[pro_index]}, {masked_name[con_index]} means the user {masked_name[con_index]} wins, {masked_name[pro_index]} means user {masked_name[pro_index]} wins. \
                    Please only give the result without any other words.'   
        ...
        with open(f'experiments/{file_prefix}_{model_name}_{args.seed}_{args.debate_num}_remove_forfeit_change_names_1-1_reversed.json', 'a') as f:
